...
export default function SearchItemWithDate({
  result,
  ...
  const [weight, setWeight] = useState(
    Math.round(result.serving_qty * 28.3495)
  );
  ...
  const weightOnChangeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newServing =
      Number(e.target.value) / Math.round(result.serving_qty * 28.3495);
    setServings(newServing);
  };

  const addToFoodLog = () => {
    router.push(
      `/profile?item=${result.nix_item_id}&quantity=${servings}&date=${query.date}`
    );
  };
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col font-lato items-center mt-12 gap-y-12">
            <div className="flex flex-row gap-x-4">
              <button
                className="flex flex-row items-center place-content-center bg-[#5D8A66] text-white h-6 w-6 rounded-sm text-2xl"
                disabled={servings <= 1}
              ...
              <input
                value={servings}
                className="w-10 rounded-sm text-center"
              ></input>
              <button
                className="flex flex-row items-center place-content-center bg-[#5D8A66] text-white h-6 w-6 text-2xl rounded-sm"
                onClick={addHandler}
              >
                +
              </button>
            ...
            <div className="flex flex-row gap-x-4">
              <input
                ...
                className="w-12 rounded-sm text-center"
              ></input>
            ...
            {session?.user ? (
              <div className="mb-12">
                <button
                  className="text-white font-bold border-2 border-white px-4 py-2"
              ...
              <div className="flex flex-col items-center gap-y-4 text-white">
                <button className="text-white text-lg border-2 border-white px-4 py-2">
                  Login to add to food log
                </button>
                or
                <button className="text-white text-lg border-2 border-white px-4 py-2">
                  Sign up
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const res = await axios.get(
    `https://trackapi.nutritionix.com/v2/search/item?nix_item_id=${query.id}`,
    {
      headers: {
